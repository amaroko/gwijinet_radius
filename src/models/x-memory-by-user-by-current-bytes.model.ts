...
@model({
  settings: {
    idInjection: false,
    mysql: {schema: 'sys', table: 'x$memory_by_user_by_current_bytes'}
  }
})
export class XMemoryByUserByCurrentBytes extends Entity {
  @property({
    type: 'string',
    length: 32,
  ...
  constructor(data?: Partial<XMemoryByUserByCurrentBytes>) {
    super(data);
  }
}

export interface XMemoryByUserByCurrentBytesRelations {
  // describe navigational properties here
}

export type XMemoryByUserByCurrentBytesWithRelations = XMemoryByUserByCurrentBytes & XMemoryByUserByCurrentBytesRelations;
